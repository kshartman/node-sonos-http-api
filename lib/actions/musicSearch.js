  ...
  return getAccountId(player, service)
    ...
    .then((resList) => {
      ...
      if (serviceDef.empty(type, resList)) {
          serviceDef = getService("library");
        return Promise.reject('No matches were found by service: ' + service);
      } else {
        var UaM = null; 
        ...
        if ((type == 'album' || type =='playlist') && (service != 'library')) {
          ...
          if (tracks.count == 0) {
            return Promise.reject('No matching tracks were loaded by service: ' + service);
          } else {
